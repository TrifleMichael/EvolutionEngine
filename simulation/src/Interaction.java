...
public class Interaction {
    ...
    public void performInteraction(Animal current, Animal enemy){
        if(betterSightThanCamouflage(current,enemy)){ // A sees B
            if(randomizedAction(current, GenomCode.AGGRESSION)){ // A wants to attack B
                if(betterSightThanCamouflage(enemy,current)){ // B sees A
                    if(randomizedAction(enemy, GenomCode.AGGRESSION)){ // B wants to attack A
                        // Both want to fight
                ...
                if(betterSightThanCamouflage(enemy,current)){ //B sees A
                    if(randomizedAction(enemy, GenomCode.AGGRESSION)){// B want to attack A
        ...
        }else if(betterSightThanCamouflage(enemy,current) && randomizedAction(enemy, GenomCode.AGGRESSION)){ // B sees A and want to attack
    ...
    private boolean randomizedAction(Animal animal, GenomCode code){
        double random = Math.random();
        return random<=animal.getGene(code);
    }
